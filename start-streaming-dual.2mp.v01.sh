...
in=$(( $1 + 100 ))
  ...
  st=0
    ...
    for i in $( seq 101 $in )
    do
	    /snap/bin/vlc -I dummy -vvv -L --syslog $(pwd)/video/h264/2Mp/one.wild.day.1080p.2mp.mkv --start-time $st --sout "#rtp{sdp=rtsp://192.168.0.$i:554/test4}" --sout-rtsp-user admin --sout-rtsp-pwd 12345678 2>/dev/null &
